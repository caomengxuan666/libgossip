# FindDependencies.cmake This module handles finding dependencies in a specific
# order: 1. System libraries 2. VCPKG libraries 3. Package manager libraries 4.
...
function(find_dependency_ordered DEPENDENCY_NAME)
  ...
  cmake_parse_arguments(FIND_DEP "${options}" "${oneValueArgs}"
                        "${multiValueArgs}" ${ARGN})
  ...
  if(NOT FIND_DEP_SYSTEM_ONLY)
    find_package(${DEPENDENCY_NAME} ${FIND_DEP_VERSION} QUIET
                 COMPONENTS ${FIND_DEP_COMPONENTS})
  ...
  # 1. Try VCPKG libraries
  if(DEFINED VCPKG_INSTALLED_DIR AND NOT FIND_DEP_VCPKG_ONLY)
    find_package(${DEPENDENCY_NAME} ${FIND_DEP_VERSION} PATHS
                 ${VCPKG_INSTALLED_DIR} QUIET COMPONENTS ${FIND_DEP_COMPONENTS})
    if(${DEPENDENCY_NAME}_FOUND)
  ...
  # 1. Try package manager libraries (pkg-config)
  ...
  if(NOT FIND_DEP_DOWNLOAD_ONLY)
    message(
      WARNING "${DEPENDENCY_NAME} not found in system, VCPKG, or pkg-config. "
...
function(find_or_fetch_googletest)
  option(USE_SYSTEM_GOOGLETEST
         "Use system-installed GoogleTest instead of downloading" OFF)
  ...
  if(USE_SYSTEM_GOOGLETEST)
    ...
    if(GTest_FOUND)
      ...
      message(
        WARNING "System GoogleTest not found, but USE_SYSTEM_GOOGLETEST is ON")
    endif()
  endif()
...
function(setup_bundled_asio)
  ...
  if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/third_party/asio)
    ...
    message(
      FATAL_ERROR
        "ASIO not found in third_party/asio. Please initialize submodules with: git submodule update --init"
    )
  endif()
endfunction()
