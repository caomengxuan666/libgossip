...
int main() {
    ...
    // Step 4: Real deployment scenario - only seed nodes are explicitly joined
    // Nodes 1 and 2 only know about node 0 (seed node)
    nodes[1]->meet(nodes[0]->self());// Node 1 joins the cluster through seed node 0
    nodes[2]->meet(nodes[0]->self());// Node 2 joins the cluster through seed node 0

    // Key fix: Let node 0 also know about nodes 1 and 2 (through anti-entropy, not manual meet)
    // But we need to let nodes 1 and 2 send messages containing themselves first

    // Step 5: Run for a sufficient time to allow nodes to automatically discover each other through gossip
    std::cout << "Starting seed-based cluster formation...\n";
    std::cout << "Only nodes 1 and 2 explicitly know about seed node 0.\n";
    std::cout << "Other connections will be discovered through gossip.\n\n";

    for (int step = 0; step < 100; ++step) {// Increase steps
    ...
    // Run 10 additional steps to ensure anti-entropy completes
    ...
    std::cout << "\nFinal cluster state:\n";
    for (int i = 0; i < N; ++i) {
        auto node_list = nodes[i]->get_nodes();
        std::cout << "Node " << (100 + i) << " knows about " << node_list.size() << " nodes:\n";
        for (const auto &n: node_list) {
            std::cout << "  Node " << static_cast<int>(n.id[15])
                      << " (" << status_str(n.status) << ")\n";
        }
    }
